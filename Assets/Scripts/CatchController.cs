...
public class CatchController : MonoBehaviour
{
    ...
    void Update()
    {
        // Para comprobar si el jugador est� dentro del square y su collider 
        jugadorDentroDelSquare = squareCollider.OverlapPoint(player.position);

        // Si mi jugador est adentro del square y el trigger "Agarre" no est� activado, entonces llamamos a activar el trigger.
        ...
        // Para este caso, suponemos que mi jugador est� dentro del square y el trigger "Agarre" est� activado, en todo caso se va activar el booleano "Agarrando".
        if (jugadorDentroDelSquare && enemyAnimator.GetBool("Agarre"))
        {
            enemyAnimator.SetBool("Agarrando", true);
        }

        // Ac� llamo a las direccion de mi enemigo dependiendo su floats como direccionales
